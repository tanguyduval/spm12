    ...
    case 'showvaledit'
        ...
        if ~isempty(contents{10})
            ...
            for iii=1:length(previewmenus)
                if ~isempty(strfind(func2str(contents{10}),'cfg_run_call_system(''save''')) || ~isempty(strfind(func2str(contents{10}),'cfg_run_call_matlab(''save'''))
                    set(previewmenus(iii),'Label','Save');
                else
                    set(previewmenus(iii),'Label','Preview');
                end
            end
        end
