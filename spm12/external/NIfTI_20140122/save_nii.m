...
function save_nii(nii, fileprefix, old_RGB)
   ...
   if strcmp(fileprefix(end-2:end), '.gz')
	 ...
	 ~strcmp(fileprefix(end-6:end), '.nii.gz')
      ...
      if ~logical(exist('OCTAVE_VERSION', 'builtin')) && (str2num(v(1:3)) < 7.1 || ~usejava('jvm'))
   ...
   %   
   if ~isempty(strfind(fileprefix,'.nii')) && strcmp(fileprefix(end-3:end), '.nii')
      filetype = 2;
      fileprefix(end-3:end)='';
   end
   
   if ~isempty(strfind(fileprefix,'.hdr')) && strcmp(fileprefix(end-3:end), '.hdr')
      fileprefix(end-3:end)='';
   end
   
   if ~isempty(strfind(fileprefix,'.img')) && strcmp(fileprefix(end-3:end), '.img')
      fileprefix(end-3:end)='';
   end
